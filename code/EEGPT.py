...
class EEGPT2DD(nn.Module):
    def __init__(self, frommodel: nn.Module):
        super(EEGPT2DD, self).__init__()
        # Base transformer model that outputs a [B, 15, 4, 512] tensor
        self.basemodel = frommodel
        # Flattened size: 15 * 4 * 512 = 30_720
        # Project to 128 * 1024 = 131_072
        self.proj = nn.Linear(15 * 4 * 512, 128 * 1024)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        B = len(x)
        x = x.view(B, -1)  # -> [B, 30720]
        # Project to target feature size
        x = self.proj(x)   # -> [B, 131072]
        # Reshape to [B, 128, 1024]
        x = x.view(B, 128, 1024)
        return x
